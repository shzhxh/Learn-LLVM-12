...
AST.h头文件声明了AST的接口，将在后面展示。LLVM的编码指南禁止使用<iostream>库，所以必须包含相同功能的LLVM头文件。需要使用输出流输出一个错误信息，让我们更详细地了解一下：\par
...
class Parser {
	...
	bool HasError;
\end{lstlisting}
Lex和Tok是上一节中类的实例。Tok存储下一个标记(前置)，而Lex用于检索下一个标记。HasError标志指示是否检测到错误。
...
\end{lstlisting}
advance()从词法分析器中检索下一个标记。在Expect()中测试，并提前检查是否为预期的类型，如果不是，则发出错误消息。最后，如果是预期的类型，consume()将检索下一个标记。如果发出错误消息，则将HasError标志设置为true。\par
